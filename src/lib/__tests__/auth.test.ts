...
const DEMO_USERS: User[] = [
  ...
  { id: 'u_adm2', name: 'Fatima',       email: 'fatima@starprogress.demo',    username: 'fatima',    passwordHash: simpleHash('Admin@2026'),     role: 'admin',       accountStatus: 'active', createdAt: '', avatarColor: '' },
  { id: 'u_p1',   name: 'Sara',         email: 'sara@starprogress.demo',      username: 'sara',      passwordHash: simpleHash('Sara@2026'),      role: 'participant', accountStatus: 'active', createdAt: '', avatarColor: '' },
  { id: 'u_p2',   name: 'Ali',          email: 'ali@starprogress.demo',       username: 'ali',       passwordHash: simpleHash('ali@2026'),       role: 'participant', accountStatus: 'active', createdAt: '', avatarColor: '' },
];
...
describe('login logic — username support', () => {
  ...
  it('logs in fatima as regular admin', () => {
    const result = loginLogic(DEMO_USERS, 'fatima', 'Admin@2026');
    expect(result.success).toBe(true);
    expect((result as { success: boolean; user: User; error?: string }).user.role).toBe('admin');
  });

  it('logs in sara as participant', () => {
    const result = loginLogic(DEMO_USERS, 'sara', 'Sara@2026');
    expect(result.success).toBe(true);
    expect((result as { success: boolean; user: User; error?: string }).user.role).toBe('participant');
  });

  it('logs in ali as participant', () => {
    const result = loginLogic(DEMO_USERS, 'ali', 'ali@2026');
    expect(result.success).toBe(true);
    expect((result as { success: boolean; user: User; error?: string }).user.role).toBe('participant');
  });
...
describe('login logic — email support', () => {
  it('also works with email address', () => {
    const result = loginLogic(DEMO_USERS, 'sara@starprogress.demo', 'Sara@2026');
    expect(result.success).toBe(true);
  });
...
describe('role-based access control expectations', () => {
  ...
  it('only main_admin has manage-admins permission', () => {
    const adminResult = loginLogic(DEMO_USERS, 'fatima', 'Admin@2026');
